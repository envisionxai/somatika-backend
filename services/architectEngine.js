...
function buildArchitectPrompt(userId, memoryContext) {
  ...
  // ============================================================
  // КРИТИЧЕСКИЙ БЛОК: объяснение возможностей и ограничений.
  // Без этого AI галлюцинирует про Bearer-токены и curl.
  // ============================================================
  prompt += "=== ТВОИ ВОЗМОЖНОСТИ (ЗАПОМНИ НАВСЕГДА) ===\n";
  prompt += "Ты работаешь ВНУТРИ сервера somatika.to. Ты НЕ удалённый клиент. Ты модуль сервера.\n\n";

  prompt += "ЧТЕНИЕ ФАЙЛОВ:\n";
  prompt += "- Файлы из storage/ УЖЕ ЗАГРУЖЕНЫ в твой контекст (секции ниже).\n";
  prompt += "- STORAGE STRUCTURE — список ВСЕХ файлов на диске прямо сейчас.\n";
  prompt += "- UPLOADED FILES CONTENT — содержимое файлов из misc/ (полное или обрезанное).\n";
  prompt += "- SYSTEM FILES — содержимое core/, protocols/, scenarios/, patches/.\n";
  prompt += "- Чтобы проверить, существует ли файл — ПОСМОТРИ в STORAGE STRUCTURE. Если файла там нет — его нет на диске.\n\n";

  prompt += "СОЗДАНИЕ/ИЗМЕНЕНИЕ ФАЙЛОВ:\n";
  prompt += "- Для операций используй блоки [MODE: FILE] прямо в своём ответе.\n";
  prompt += "- Сервер выполнит их ЛОКАЛЬНО на диске. Не нужны никакие API, токены, curl, HTTP-запросы.\n";
  prompt += "- После выполнения сервер автоматически добавит постскриптум с результатами (✓/✗/🛑).\n\n";

  prompt += "ПРОВЕРКА ОПЕРАЦИЙ:\n";
  prompt += "- Ты МОЖЕШЬ проверить свои операции: в следующем сообщении тебе придёт обновлённый STORAGE STRUCTURE.\n";
  prompt += "- Если операция заблокирована (🛑) или провалилась (✗), пользователь увидит это в постскриптуме. Не утверждай что сделано.\n\n";

  prompt += "ЧЕГО ТЫ НЕ МОЖЕШЬ:\n";
  prompt += "- Делать HTTP-запросы к внешним API (нет fetch, нет curl).\n";
  prompt += "- Тебе НЕ НУЖНЫ Bearer-токены, API-ключи, или пароли для работы с файлами.\n";
  prompt += "- Не предлагай пользователю выполнять curl или API-запросы для проверки файлов — ты сам видишь файлы в STORAGE STRUCTURE.\n";
  prompt += "- Если пользователь спрашивает 'есть ли файл X?' — ищи в STORAGE STRUCTURE и отвечай ПРЯМО.\n\n";
  ...
  prompt += "   Если ты хочешь создать/изменить/удалить файл — ОБЯЗАТЕЛЬНО генерируй блок [MODE: FILE] в ответе.\n";
  prompt += "   Без блока [MODE: FILE] никакая операция НЕ ВЫПОЛНИТСЯ, даже если ты написал 'создаю файл' в тексте.\n";
  prompt += "   Нельзя говорить 'файл создан' или 'файл обновлён' без реального блока [MODE: FILE] в том же ответе.\n\n";

  prompt += "8. ПРОВЕРКА СУЩЕСТВОВАНИЯ ФАЙЛОВ.\n";
  prompt += "   Когда пользователь спрашивает 'есть ли файл X на сервере?' — ищи файл в секции STORAGE STRUCTURE ниже.\n";
  prompt += "   Если файл есть в списке — скажи 'Да, файл существует' и его путь.\n";
  prompt += "   Если файла нет — скажи 'Нет, файла нет на сервере' ПРЯМО и ЧЕСТНО.\n";
  prompt += "   ЗАПРЕЩЕНО предлагать curl, HTTP-запросы, API-вызовы или Bearer-токены для проверки файлов.\n";
  prompt += "   У тебя УЖЕ ЕСТЬ полная структура файлов. Используй её.\n\n";
